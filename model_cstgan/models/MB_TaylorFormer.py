...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        out_denominator = torch.full(
            (h*w, c//self.num_heads), h*w, device=q.device
        ) + q @ k_sum.expand(-1, -1, -1, c//self.num_heads) + torch.tensor(1e-6, device=q.device)
...
class MHCAEncoder(nn.Module):
    ...
    def forward(self, x, size):
        ...
        x = x.reshape(B, H, W, -1).contiguous().permute(0, 3, 1, 2)
...
class DWConv2d_BN(nn.Module):
    ...
    def __init__(
       ...
       #                                                 stride=1, padding=0, bias=False)
        ...
        if in_ch == 1:
            self.dcn = nn.Conv2d(in_ch, in_ch, 3, 1, 1, bias=False)
        else:
...
class MB_TaylorFormer(nn.Module):
    def __init__(self,
        ...
        self.output = nn.Sequential(  # nn.Conv2d(n_feat, n_feat*2, kernel_size=3, stride=1, padding=1, bias=False),
            # nn.BatchNorm2d(dim*2),
            # nn.Hardswish(),
            nn.Conv2d(dim[1], 1, kernel_size=3, stride=1, padding=1, bias=False, ),
