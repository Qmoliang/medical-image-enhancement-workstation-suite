...
该仓库并不是一个面向完整流程复现的公开 release，而是一个用于说明系统结构、工程组织方式和本地工作流的研究代码仓库。
...
虽然本仓库包含模型工程目录，但不包含实际权重和数据，因此当前公开版本主要用于展示代码结构、页面组织、接口设计和本地工作流编排方式。完整结果复现仍依赖额外的模型权重、原始数据和本地运行环境。
